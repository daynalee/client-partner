...
export function buildTalkingPoints(client: Client, insights: ClientInsights): string[] {
  ...
  if (best && best.spendShare < 0.2) {
    ...
    points.push(
      `${best.campaign.name} continues to lead at ${fmtRoas(best.current.roas)} ROAS on ${fmtUSD(best.current.spend)} — it's carrying the account and has room to keep scaling.`,
    )
  }
  ...
  if (fatigue) {
    points.push(
      `One thing I'm watching closely: ${fatigue.whatsHappening.split('—')[0].trim()}. This is a creative problem, not a media problem — a refresh sprint fixes it, and I'd rather do that now than discount our way through Q4.`,
    )
  }
  ...
  points.push(
    `On seasonality: shoppers here plan 2–3 months ahead. Decisions we make in the next two weeks determine whether we own Holiday discovery or pay peak prices to rent it in November.`,
  )
...
export function buildFollowUpEmail(client: Client, insights: ClientInsights): string {
  ...
  return `Subject: ${client.name} — performance recap & next steps
