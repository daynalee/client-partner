...
export function analyzeClient(client: Client): ClientInsights {
  ...
  for (const r of rows) {
    ...
    if (decline <= -0.18 && r.spendShare >= 0.1) {
      opportunities.push({
        ...
        action: `Ship a refresh sprint: 6–8 new assets mixing seasonal angles and video formats, retire the bottom-quartile Pins, and move to a 4–6 week creative rotation cadence.`,
  ...
  for (const r of rows) {
    if (r.momSpend > 0.25 && r.momRoas < -0.08) {
      ...
      opportunities.push({
        ...
        expectedImpact: `Protects the scale trajectory while establishing the efficiency floor — informs whether the next ${fmtUSD(summary.monthlySpend * 0.15)}/month of budget should go here or to a new audience.`,
      })
      risks.push({
  ...
  for (const r of rows) {
    ...
    if ((r.current.roas >= blendedRoas * 1.4 || r.spendShare > 0.35) && impGrowth < 0.08) {
      opportunities.push({
        ...
        expectedImpact: `A 25% larger visitor pool compounds through retargeting within 4–6 weeks — growth in the account's most efficient channel without bidding against saturation.`,
      })
      break
    }
  }
  ...
  if (summary.cvr < 0.016 && summary.ctr >= 0.007) {
    opportunities.push({
      ...
      tag: 'Measurement',
      priority: 'High',
      whatsHappening: `Account CTR is healthy at ${fmtPct(summary.ctr, 2)}, but conversion rate is only ${fmtPct(summary.cvr, 1)} — well below the 2.5–3.5% typical for comparable apparel advertisers with this level of engagement.`,
      ...
      expectedImpact: `Comparable audits typically recover 15–30% of unattributed conversions — which could move reported ROAS from ${fmtRoas(summary.roas)} to ~${fmtRoas(summary.roas * 1.2)} with zero media changes.`,
    })
    risks.push({
      id: 'risk-tracking',
  ...
  for (const r of rows) {
    ...
    if (r.current.roas < blendedRoas && r.momRevenue > 0.12) {
      opportunities.push({
        ...
        expectedImpact: `Establishes the real value of upper-funnel spend and typically justifies a 20–40% higher prospecting budget than last-click reporting alone would support.`,
      })
      break
    }
  }
  ...
  for (const r of rows) {
    if (
      ...
      opportunities.push({
        ...
        expectedImpact: `Broader catalog coverage typically adds 10–20% incremental shopping revenue at similar efficiency within two months.`,
      })
      break
    }
  }
  ...
  for (const r of rows) {
    if (
      ...
      opportunities.push({
        ...
        action: `Pull forward planned seasonal budget — step this campaign up 20–25% per week while ROAS holds above target, and extend top assets into additional formats to capture remaining planner demand.`,
        expectedImpact: `Capturing the seasonal curve early typically delivers 15–25% cheaper conversions than equivalent spend at peak, and builds engagement pools that retargeting monetizes through the season's tail.`,
      })
      break
    }
  }
  ...
  // --- Seasonal calendar (users plan 2–3 months ahead of the moment)
  ...
  const seasonal: SeasonalOpportunity[] = [
    ...
    },
    {
      ...
      window: 'Planning now, purchase peak Sep–Oct',
      insight: 'Shoppers on visual discovery platforms start saving fall outfit ideas 2–3 months before they buy. Fall creative running in August reaches planners before intent gets expensive.',
    },
    {
      moment: 'Holiday gifting',
      window: 'Planning begins Sep, purchase peak Nov–Dec',
